...
- [x] Настройка FastAPI сервера
- [x] Создание таблиц в PostgreSQL
- [x] Наполнение таблицы `cities` (первые 50 городов)
- [x] Реализация API для игры (start, submit-guess)
...
## 12. MVP
Создать проект, в котором пользователь сможет угадывать города по всему миру.
В нашем приложение должна быть возможность получить город и попробовать указать точную позицию.
Также пользователь сможет получить подсказку, если ему будет слишком сложно!

-- авторизация и многопользовательская история. ui
-- только географические подсказки перегенерировать! без столиц город